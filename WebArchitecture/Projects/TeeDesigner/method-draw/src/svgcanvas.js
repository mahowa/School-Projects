...
this.getLayers = function(){
  return getCurrentDrawing().all_layers;
};
...
this.moveSelectedToLayer = function(layername) {
  ...
  this.mergeLayersList = function(selected){
    var batchCmd = new BatchCommand("Merge Layer");

    var prev, next;
    var drawing = getCurrentDrawing();
    for (var i = 0; i < drawing.getNumLayers(); ++i) {
      if (drawing.getLayerName(i) == selected[0]) {
        prev = drawing.all_layers[i][1];
        continue;
      }
      if (drawing.getLayerName(i) == selected[1]) {
        next = drawing.all_layers[i][1];
      }
    }

    var layerNextSibling = next.nextSibling;
    batchCmd.addSubCommand(new RemoveElementCommand(next, layerNextSibling, svgcontent));

    while(next.firstChild) {
      var ch = next.firstChild;
      ...
      prev.appendChild(ch);
      batchCmd.addSubCommand(new MoveElementCommand(ch, oldNextSibling, next));
    }

    // Remove current layer
    svgcontent.removeChild(next);

    var v = $('#LayerList').get()[0];
    for(i=0; i< v.childNodes.length; i++){
      if(v.childNodes[i].innerText == selected[1])
        v.removeChild(v.childNodes[i]);
    }

    var index = selected.indexOf(selected[1]);
    if(index >= 0){
