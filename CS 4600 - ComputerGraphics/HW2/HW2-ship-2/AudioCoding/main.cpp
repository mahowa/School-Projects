...
//#define WAV_FILE "data/handel.wav"
...
{
	// TODO: part of Homework Task 1
	// takes a vector y and produce as output a vector x,  where the kth element of the vector can be computed from x_k = q_k *(dot) y
...
void compressWAVSignal()
{
	...
	//CODE TO OUTPUT Q for Visual Debugging
	for (int i = 0; i < 8; i++){
		for (int j = 0; j < 8; j++){
			if (q[(i * 8) + j] > 0)
				std::cout << " ";
